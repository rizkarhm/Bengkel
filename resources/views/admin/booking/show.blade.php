        ...
        <div class="col-12">
            <div class="card shadow mb-4">
                    ...
                    <div class="form-group">
                        ...
                        <select name="user_id" id="user_id" class="custom-select" disabled>
                            ...
                            @foreach ($cust as $customer)
                                <option value="{{ $bookings->user_id }}" @selected(isset($bookings) ? $customer->id == $bookings->user_id : '')>{{ $customer->nama }}</option>
                            @endforeach
                        </select>
                        @error('user_id')
                    ...
                    <div class="form-group">
                        ...
                        <select name="kendaraan_id" id="kendaraan_id" class="custom-select" disabled>
                            ...
                            @foreach ($kendaraans as $merek)
                                <option value="{{ $bookings->kendaraan_id }}" @selected(isset($bookings) ? $merek->id == $bookings->kendaraan_id : '')>{{ $merek->merek }}</option>
                            @endforeach
                        </select>
                        @error('kendaraan_id')
                            <span class="text-danger">{{ $message }}</span>
                ...
                <div class="card-footer">
                    <a href="{{ route('booking.edit', $bookings) }}" class="btn btn-warning">Edit Appointment</a>
                </div>
            </div>
        </div>
    </div>
